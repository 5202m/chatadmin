      ...
      <tr>
        ...
        <td width="35%" colspan="3">
        	<select id="status" name="status" style="width: 155px;">
        		<option value="1" <c:if test="${advertisement.status == 1}">selected</c:if>>启用</option>
         		<option value="0" <c:if test="${advertisement.status == 0}">selected</c:if>>禁用</option>
         	</select>
        </td>
      </tr>
      <tr>
        ...
        <td width="35%" colspan="3">
        	<div>
	        	...
	        	<!-- 裁剪后图片的路径 -->
	        	<input type="hidden" id="cutedImagePath" value="${advertisement.img}"/>
	        	<!-- 表单提交时保存到数据库的字段-->
	        	<input type="hidden" name="img" id="saveImagePath" value="${advertisement.img}"/>
      ...
      <tr>
        ...
        <td width="35%" colspan="3">
        	<input type="text" name="imgUrl" id="imgUrl" style="width: 800px;" class="easyui-validatebox" 
		          	    data-options="required:true,missingMessage:'请输入广告链接URL'" value="${advertisement.imgUrl}"/>
        </td>
      </tr>
      <tr style="margin-top: 6px;">
	     <td colspan="6" align="right">&nbsp;&nbsp;
		     <a href="#" class="easyui-linkbutton" id="advertisement_edit_b1_submit" onclick="advertisementEdit.onSaveEdit()" data-options="iconCls:'ope-submit'" >提交</a> &nbsp;&nbsp;
		     <a href="#" class="easyui-linkbutton" id="advertisement_edit_b1_back" onclick="advertisement.back()" data-options="iconCls:'ope-previous'" >返回</a>
		 </td>
      </tr>
    </table>
  </form>
