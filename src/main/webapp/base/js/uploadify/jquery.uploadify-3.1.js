...
(function($) {
			...
			this
					.each(function() {
						...
						if (args[0]) {
							...
							$item.delay(200).fadeOut(100, function() {
								$(this).remove();
							});
						}
					});
	...
	var handlers = {
		...
		onSelect : function(file) {
			...
			var suffix = 'KB';
			if (fileSize > 200) {
				fileSize = Math.round(fileSize / 200);
				suffix = 'MB';
			}
		...
		onUploadProgress : function(file, fileBytesLoaded, fileTotalBytes) {
			...
			var suffix = 'KB/s';
			var mbs = 0;
			var kbs = (lapsedBytes / 1024) / (lapsedTime / 200);
			...
			if ($.inArray('onUploadProgress', settings.overrideEvents) < 0) {
				if (settings.progressData == 'percentage') {
					$('#' + file.id).find('.data').html(
							' - ' + percentage + '%');
				} else if (settings.progressData == 'speed' && lapsedTime > 100) {
					$('#' + file.id).find('.data').html(
