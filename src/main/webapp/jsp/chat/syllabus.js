    ...
    setEditEvent : function() {
        ...
        $("#panel_editSyllabus tbody select[name='status']").each(function(index){
        	$(this).bind("change", index, function (event, status) {
                ...
                loc_panel.find("select[name='lecturer']").eq(loc_index).prop("disabled", loc_disable);
		...
		goldOfficeUtils.ajax({
			...
			success : function(data){
				...
				if(loc_syllabus && loc_syllabus.courses)
				{
					...
					$("#panel_viewSyllabus div:last").html(Syllabus.viewSyllabus(loc_syllabus.courses, loc_currTime, loc_style));
				}
				else
				{
        ...
        $panel.find("tbody:visible").each(function(){
            ...
            for(var i = 0; i < lenI; i++)
            {
                ...
                loc_courses.push({
                    ...
                    lecturer : loc_this.find("select[name='lecturer']").eq(loc_index).val() || "",
        ...
        loc_newTimeBucket.find("td").each(function(index){
            ...
            if(loc_courses.hasOwnProperty(loc_day)){
                ...
                if(index < 7){
                    ...
                    $(this).find("select[name='lecturer']").val(loc_course.lecturer);
                }else{
