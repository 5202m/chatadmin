...
public class TopicController extends BaseController {
	...
	public AjaxJson create(HttpServletRequest request, Topic topic) {
		...
		String tempSubject = request.getParameter("tempSubject"), tempProduct = request.getParameter("tempProduct");
	...
	public String reply(HttpServletRequest request, ModelMap map, DataGrid dataGrid, Reply reply) throws Exception {
		dataGrid.setOrder("desc");
		dataGrid.setSort("replyId");
		reply.setType(1);
		Page<Reply> page = replyService.getReplyPage(this.createDetachedCriteria(dataGrid, reply));
		map.put("total", null == page ? 0 : page.getTotalSize());
		map.put("pageNumber", dataGrid.getPage());
		map.put("pageSize", dataGrid.getRows());
		map.put("replyList", null == page ? new ArrayList<Reply>() : page.getCollection());
	 ...
	 * 功能：帖子管理-删除回帖
	 */
	@RequestMapping(value = "/topicController/replyDel", method = RequestMethod.POST)
	@ResponseBody
	@ActionVerification(key = "delete")
	public AjaxJson replyDel(HttpServletRequest request) {
		...
		if (result.isOk()) {
			...
			logService.addLog(message, WebConstant.Log_Leavel_INFO, WebConstant.Log_Type_DEL, BrowserUtils.checkBrowse(request), IPUtil.getClientIP(request));
			logger.info("<<method:replyDel()|" + message);
		}
		else {
			j.setSuccess(false);
			j.setMsg(ResourceBundleUtil.getByMessage(result.getCode()));
			String message = " 用户: " + userParam.getUserNo() + " " + DateUtil.getDateSecondFormat(new Date()) + " 删除帖子回复失败";
			logService.addLog(message, WebConstant.Log_Leavel_ERROR, WebConstant.Log_Type_DEL, BrowserUtils.checkBrowse(request), IPUtil.getClientIP(request));
			logger.error("<<method:replyDel()|" + message + ",ErrorMsg:" + result.toString());
		}
		return j;
	}
}
